...
if [ "$command" = "esp_jtag" ]; then
	...
	esptool.py -p $port -b $baude write_flash 0x00000 esp_jtag.bin
fi
...
if [ "$command" = 'esp_nodemcu' ]; then
	...
	esptool.py --port $port --baud $baude write_flash --flash_size=detect -fm dio 0 nodemcu/nodemcu-float.bin
fi

if [ "$command" = 'esp_ap' ]; then
	echo "Esp como Access Point final"
	esptool.py -p $port -b $baude write_flash 0x00000 esp_ap.bin
fi
...
if [ "$command" = 'help' ]; then
	...
	echo "====="
	echo ""
	echo "./mediaLabInstall.sh [command] [port] [baud]"
	echo ""
	echo "command: [esp_jtag] [esp_erase] [micropython] [esp_ap] [help]"
	echo "port: [/dev/ttyUSB0] [/dev/ttyUSB1] ..."
	echo "baude: [115200] [230400] ..."
	echo ""
	echo "Ejemplo:"
	echo "./mediaLabInstall.sh esp_jtag ttyUSB0 230400"
	echo ""
	echo ""
	echo "INSTALAR SOFTWARE EN MEDIA LAB"
	echo "================================="
	echo ""
